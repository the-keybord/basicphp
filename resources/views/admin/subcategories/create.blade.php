        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-8 border-b border-gray-50 dark:border-zinc-800/60 bg-gray-50/50 dark:bg-zinc-950/20">
                    <h3 class="text-lg font-black text-gray-900 dark:text-white">Subcategory Details</h3>
                    <p class="text-sm text-gray-500 dark:text-neutral-450 mt-1">Select a parent category and configure sub-level default constraints.</p>
                </div>

                <form action="{{ route('admin.subcategories.store') }}" method="POST" class="p-8 space-y-6">
                    @csrf

                    <!-- Parent Category -->
                    <div class="space-y-1.5">
                        <label for="category_id" class="text-xs font-bold text-gray-400 dark:text-neutral-400 uppercase tracking-widest">Parent Category</label>
                        <select 
                            name="category_id" 
                            id="category_id" 
                            required
                        ...
                        >
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" @selected(old('category_id', $selectedCategoryId) == $cat->id)>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Name -->
                    <div class="space-y-1.5">
                        <label for="name" class="text-xs font-bold text-gray-400 dark:text-neutral-400 uppercase tracking-widest">Subcategory Name</label>
                        <input 
                            type="text" 
                            name="name" 
                            id="name" 
                            value="{{ old('name') }}" 
                            required
                            placeholder="e.g. data retrieval, tables, constraints"
                            class="block w-full border-gray-200 dark:border-zinc-850 dark:bg-zinc-950/40 rounded-xl p-3.5 text-sm text-gray-700 dark:text-white font-medium focus:ring-blue-500 focus:border-blue-500 transition shadow-sm"
                        >
                        @error('name')
                            <p class="text-red-500 text-xs font-semibold mt-1">{{ $message }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input 
                                ...
                                value="{{ old('default_test_size', 8) }}"
                                min="1"
                                max="100"
                                class="block w-full border-gray-200 dark:border-zinc-850 dark:bg-zinc-950/40 rounded-xl p-3.5 text-sm text-gray-700 dark:text-white font-medium focus:ring-blue-500 focus:border-blue-500 transition shadow-sm"
                            >
                            <span class="text-xxs text-gray-400">Default questions selected from this subcategory during automated test creation.</span>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input 
                                ...
                                value="{{ old('default_test_time', 45) }}"
                                min="1"
                                max="300"
                                class="block w-full border-gray-200 dark:border-zinc-850 dark:bg-zinc-950/40 rounded-xl p-3.5 text-sm text-gray-700 dark:text-white font-medium focus:ring-blue-500 focus:border-blue-500 transition shadow-sm"
                            >
                            <span class="text-xxs text-gray-400">Default time duration allocated for this subcategory's sections.</span>
