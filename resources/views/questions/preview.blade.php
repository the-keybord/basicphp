<x-app-layout>
    ...
    <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row gap-6 items-start">
            ...
            <!-- Left Sidebar (Title, Navigation, Mapping, Configurator, Stored Key) -->
            <aside class="w-full md:w-80 md:sticky md:top-6 space-y-6 flex-shrink-0">
                ...
                <div class="bg-white rounded-2xl border border-gray-150 p-5 shadow-sm space-y-4">
                    <div class="flex items-center space-x-3 border-b border-gray-100 pb-3">
                        <a href="{{ route('admin.questions.index') }}" class="text-gray-500 hover:text-gray-800 transition flex-shrink-0">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <span class="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-lg text-xxs font-semibold uppercase tracking-wider">
                            ID: #{{ $question->id }}
                        </span>
                        @php
                            $badgeColors = [
                        ...
                        @endphp
                        <span class="px-2.5 py-1 border rounded-lg text-xxs font-semibold capitalize {{ $color }}">
                            {{ $formattedType }}
                        </span>
                    </div>

                    <div class="pt-2 border-t border-gray-100">
                        <form action="{{ route('admin.questions.clone', $question) }}" method="POST" class="w-full">
                            @csrf
                            <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold uppercase rounded-lg shadow-sm transition gap-1.5">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl p-4.5 shadow-md space-y-3.5">
                    <div class="flex items-center space-x-3">
                        <div class="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center border border-white/20 flex-shrink-0">
                        ...
                        <div>
                            ...
                            <span class="text-[10px] text-blue-100/80">Press ← / → keys to navigate.</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        @if($prevId)
                            <a id="prev-question-btn" href="{{ route('admin.questions.preview', $prevId, false) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-white/10 hover:bg-white/20 border border-white/10 text-white rounded-xl text-xs font-bold uppercase transition shadow-sm">
                                ← Prev
                            </a>
                        @else
                            <button disabled class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-white/5 text-white/30 border border-white/5 rounded-xl text-xs font-bold uppercase cursor-not-allowed">
                                ← Prev
                            </button>
                        @endif

                        @if($nextId)
                            <a id="next-question-btn" href="{{ route('admin.questions.preview', $nextId, false) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-white text-blue-600 hover:bg-blue-50 text-white rounded-xl text-xs font-extrabold uppercase transition shadow-md">
                                Next →
                            </a>
                        @else
                            <button disabled class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-white/5 text-white/30 border border-white/5 rounded-xl text-xs font-bold uppercase cursor-not-allowed">
                ...
                <form id="mapping-form" action="{{ route('admin.questions.update-mapping', $question) }}" method="POST" class="bg-white rounded-2xl border border-gray-150 p-5 shadow-sm space-y-4">
                    @csrf
                    <span class="text-xs font-bold text-gray-450 uppercase tracking-widest block border-b border-gray-100 pb-2">Category Mapping</span>
                    
                    <!-- Primary Subcategory Select -->
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Primary Mapping</label>
                        <select name="primary_subcategory_id" class="block w-full border-gray-300 rounded-xl p-2.5 pl-3 pr-8 text-xs text-gray-700 font-medium bg-white focus:ring-blue-500 shadow-sm w-auto max-w-full">
                            @foreach($categories as $category)
                                <optgroup label="{{ $category->name }}">
                                    @foreach($category->subcategories as $sub)
                                        <option value="{{ $sub->id }}" @selected($sub->id === $question->primary_subcategory_id)>
                                            {{ $sub->name }}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <!-- Secondary Subcategory Select -->
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Secondary Mapping</label>
                        <select name="secondary_subcategory_id" class="block w-full border-gray-300 rounded-xl p-2.5 pl-3 pr-8 text-xs text-gray-700 font-medium bg-white focus:ring-blue-500 shadow-sm w-auto max-w-full">
                            <option value="">None</option>
                            @foreach($categories as $category)
                                <optgroup label="{{ $category->name }}">
                                    @foreach($category->subcategories as $sub)
                                        <option value="{{ $sub->id }}" @selected($sub->id === $question->secondary_subcategory_id)>
                                            {{ $sub->name }}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div class="pt-2 border-t border-gray-100">
                        <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold uppercase rounded-lg shadow-sm transition gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Configurator / Save Answer Card -->
                <div class="bg-white rounded-2xl border border-gray-150 p-5 shadow-sm space-y-4">
                    <span class="text-xs font-bold text-gray-450 uppercase tracking-widest block border-b border-gray-100 pb-2">Configurator</span>
                    <div class="space-y-1.5">
                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Live Configured Key:</span>
                        <div id="answer-display-text" class="text-xs font-black text-blue-600 bg-blue-50 px-2.5 py-2 rounded-lg border border-blue-100 font-mono break-all text-center">
                            {{ $question->correct_answer_string ?: 'No answer selected' }}
                        </div>
                    </div>
                    
                    <div class="flex flex-col gap-2 pt-2 border-t border-gray-100">
                        <a href="{{ route('admin.questions.edit', $question) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold uppercase rounded-lg transition border border-gray-200 gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                            Edit Question
                        </a>
                        <button type="submit" form="answer-form" class="w-full inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-bold uppercase rounded-lg shadow-sm transition gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                            </svg>
                            Set Correct Answer
                ...
                <!-- Teacher Answer Key / XML Code Card -->
                <div class="bg-gray-800 rounded-2xl border border-gray-900 text-white overflow-hidden shadow-md">
                    <button type="button" onclick="toggleAnswer()" class="w-full px-5 py-3 flex items-center justify-between font-semibold text-left focus:outline-none hover:bg-gray-750 transition text-sm">
                    ...
                    <div id="answer-content" class="hidden px-5 pb-5 pt-2 border-t border-gray-700/50 bg-gray-850/50 text-gray-300 text-xs space-y-4">
                        <div>
                            <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">Stored Answer String</span>
                            <div class="text-sm font-bold text-white bg-gray-900/60 p-2.5 rounded-lg border border-gray-700/30 inline-block font-mono break-all">
                                {{ $question->correct_answer_string ?: 'No answer specified' }}
                            </div>
                        </div>

                        <div>
                            <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">Raw XML Store</span>
                            <pre class="bg-gray-900/80 p-3 rounded-lg font-mono text-[10px] text-gray-400 overflow-x-auto border border-gray-700/30 whitespace-pre-wrap leading-relaxed">{{ $question->xml_content }}</pre>
                        </div>
                    </div>
                </div>
            </aside>

            <!-- Right Main Column (Mockup Form) -->
